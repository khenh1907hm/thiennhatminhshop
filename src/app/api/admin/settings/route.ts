...
function runtimeStatus() {
  return {
    siteUrl: process.env.NEXT_PUBLIC_SITE_URL || process.env.NEXTAUTH_URL || "https://thiennhatminhshop.vercel.app",
...
export async function GET() {
  ...
  try { return NextResponse.json({ ...parse(await prisma.setting.findMany({ where: { key: { in: keys } } })), runtimeStatus: runtimeStatus() }); } catch { return NextResponse.json({ error: "Không thể tải cài đặt" }, { status: 500 }); }
}
export async function PUT(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ ...next, runtimeStatus: runtimeStatus() });
